    ...
    public class UpdateHelper
    {
        ...
        {            
            ...
            if (lastcheck != string.Empty)
            {
                ...
                if (dtlast.AddDays(7).CompareTo(DateTime.Now) <= 0)
                {
        ...
        static void bwCheckVersion_ProgressChanged(object sender, System.ComponentModel.ProgressChangedEventArgs e)
        {
            ...
            if (res.OutputMessage != String.Empty)
            {
                ...
                {
                    //3DialogResult resq = Module.ShowQuestionDialogYesFocus(TranslateHelper.Translate("A new version has been released. Would you like to download it now ?"), TranslateHelper.Translate("Download new version ?"));
